...
Up to 20 multi-function GPIOs are available on the header pins (6 can be used as ADC inputs). These GPIOs operate at 3.3V logic level and can be mapped to various digital peripherals (UART, SPI, I2C, PWM, etc.) for flexible development.
...
void loop()
{
  ...
  float voltage = vmeas * 1.77; // Adjusted for the voltage divider, assuming r1 and r2 are not used here
