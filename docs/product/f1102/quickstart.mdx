...
|---------------------------------------------------|-----------------|
| SoC in sleep mode, all peripherals off            | \<14mA          |
| SoC in active mode, all peripherals on, SX1262 RX | \<40mA          |
| SoC in active mode, all peripherals on, SX1262 TX | \<140mA         |
...
Before starting to use the FoBE Quill ESP32-S3 Mesh, please make sure to prepare the following necessary items:
...
void loop() {
  int status = digitalRead(CHARGING_STATUS_PIN);
