...
const FLASH_WORD_WRITE_TIME = 0.0001; // nRF52840 max write time
...
export class Dfu {
  ...
  async getAck() {
    ...
    if (this.lastAck !== -1 && ack !== (this.lastAck + 1) % 8) {
      ...
      throw new Error(
        `Invalid ACK sequence. Expected ${(this.lastAck + 1) % 8}, got ${ack}`,
      );
    }
    this.lastAck = ack;
    ...
    while (i < data.length) {
      if (data[i] === 0xdb) {
        ...
        if (data[i] === 0xdc) {
          ...
          throw new Error(
            `Invalid SLIP escape sequence: DB followed by ${data[i].toString(16)}`,
          );
        }
      } else if (data[i] === 0xc0) {
  ...
  async sendStartDfu(
    ...
    const eraseWaitTime = Math.max(
      0.5,
      (totalSize / FLASH_PAGE_SIZE + 1) * FLASH_PAGE_ERASE_TIME,
    );
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (filename === "manifest.json") {
          ...
          firmwareFiles[filename] = await entry.getData(
      ...
      if (
        !firmwareFiles[manifest.manifest.application.bin_file] ||
